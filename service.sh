...
if [ -d /dev/cpuctl/ ]; then
    ...
    for cpuctl_dir in /dev/cpuctl/*; do
        ...
        lock_value "1024" "${cpuctl_dir}/cpu.shares"
    done
fi
...
for cpuset_cpus in /dev/cpuset/*/cpus; do
    lock_value "${all_cpus}" "${cpuset_cpus}"
done
...
if [ -d ${POLICY2_PATH} ] && [ -d ${POLICY4_PATH} ] && [ -d ${POLICY7_PATH} ]; then #2+2+3+1
...
if [ -d /sys/class/kgsl/kgsl-3d0 ]; then
    ...
    MIN_PWRLEVEL=$(expr $(cat /sys/class/kgsl/kgsl-3d0/num_pwrlevels) - 1)
...
# else
 # exit 0
# fi
killall -15 CuAtattach
nohup $MODDIR/CuAtattach > /dev/null 2>&1 &
